...
// Reusable Ad Component to safely render the scripts
const AdComponent = ({ adKey, width, height, className = "" }: AdComponentProps) => {
  ...
  return (
    <div className={`flex flex-col items-center gap-2 ${className}`}>
       ...
       <div ref={adRef} className="bg-slate-100 rounded-lg overflow-hidden shadow-sm flex items-center justify-center" style={{ width: `${width}px`, height: `${height}px` }} />
    </div>
  );
};
...
const NotebookSketchApp = () => {
  ...
  return (
    <div className="min-h-screen font-sans text-slate-800 flex flex-col items-center p-4 sm:p-6" style={{ backgroundColor: '#fdfbf7' }}>
      ...
      {/* Responsive Horizontal Ads (Leaderboard & Mobile Banner) */}
      <div className="w-full flex justify-center mb-8 px-4 z-10">
        {/* Mobile Banner: Visible on mobile (< 768px) */}
        <div className="block md:hidden">
           <AdComponent adKey="fa68c2351b36dacaea35e1a3b3ece31a" width={320} height={50} />
        </div>
        {/* Leaderboard: Visible on tablet/desktop (>= 768px) */}
        <div className="hidden md:block">
           <AdComponent adKey="4504fee671edf465482da6b2f3700eda" width={728} height={90} />
        </div>
      </div>
      ...
      <div className="w-full flex flex-col lg:flex-row items-start justify-center gap-6">
        ...
        {/* Ad Container (Right side on large screens: 160x600) */}
        <div className="hidden lg:block">
          <AdComponent adKey="27ae0b7e0c421f82c705d8ac006e20fa" width={160} height={600} />
        </div>
      </div>
